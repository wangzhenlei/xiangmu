...
(function($) {     
    ...
    return this.each(function() {  
        ...
        $selectTit=$(opts.selectTit,$this),
        $formOption=$(opts.formOption,$this),
        $formOptionLi=$(opts.formOptionLi,$this);

        $selectTit.on(opts.eventType,function(event){
            $formOption.show();  
            event.stopPropagation();          
        });

        $formOptionLi.on(opts.eventType,function(){
            $formOption.hide(); 
            $(this).addClass('active').siblings().removeClass();
            $this.find('input').val($(this).find('a').html()); 
            $selectTit.find('span').html($(this).find('a').html()); 
        });
    });
    // 保存JQ的连贯操作结束
    };
    ...
    // 插件的defaults     
    $.fn.select.defaults = {     
        selectObj:'.form-select',
        selectTit:'.form-sltitle',
        formOption:'.form-option',
        formOptionLi:'.form-option li',
        currentClass:'active',
        eventType:'click'
    };  
    //select 用法例：$(".form-select").select({selectObj:".form-select",selectTit:".form-sltitle span",
        //formOption:".form-option",formOptionLi:".form-option li"}); 
    ...
    return this.each(function() {  
        ...
        $this.click(function(){
            ...
            if($this.find('input').attr('checked')){
                $(this).addClass('ui_radio_active').siblings().removeClass('ui_radio_active');
            } 
        });
...
function placeholderFn(){
    ...
    $('input[placeholder]').focus(function(){
        ...
        var placeholderVal='';
        _val = $(this).val();
        placeholderVal = $(this).attr('placeholder');        if(_val == ''){
            $(this).val(placeholderVal);
        }
    })
}

function lightBtn(inp,btn){
    if($(inp).val()!=''){
